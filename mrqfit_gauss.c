/*************************************************************************** 
MRQFIT_GAUSS: This is the fgauss algorithm taken from Numercial
...
3) Have added two more parameters per gaussian: 4th parameter is the
   continuum level and the 5th parameter is the continuum slope. Therefore,
   the user must make sure there are na/5 gaussians being fit.
4) To clarify, the parameters in a are defined with respect to the
   following equation: y(x) = a0*exp[-(x-a1)^2/2*a2^2] + a3*(x-a1) + a4
5) Have added an na*2 matrix additional argument defining the limits of the
   parameters.
6) If the dyda array is passed as NULL then no attempt is made to fill
   it with the derivatives.
7) Spheres of influence can be specified for each set of parameters
...
int mrqfit_gauss(double *x, double *a, double **lim, int *ia, int **inf,
  ...
  double xma1=0.0,xma12=0.0,sig2=0.0,fac=0.0,expon=0.0,ex=0.0;
  int    ninc=0,nset=0;
  int    i=0,j=0,k=0;
  int    *inc=NULL;
  
  /* Check to make sure only 1 x value has been entered */
  if (nx!=1) {
    errormsg("mrqfit_gauss(): Only 1 x-value should be provided.\n\
\tYou provided %d",nx); return 0;
  }

  /* Check to make sure number of parameters is divisible by 5 */
  if ((MOD(na,5))) errormsg("mrqfit_gauss(): Number of input parameters (=%d)\n\
\tnot a multiple of 5",na);

  /** Make some checks on the spheres of influence of each parameter set **/
  /* Initialise inc */
  ninc=0; nset=na/5;
  if ((inc=iarray(nset))==NULL)
    errormsg("mrqfit_gauss(): Cannot allocate memory for inc\n\
\tarray of size %d",nset);
  ...
  for (i=0,k=0,*y=0.0; i<na-1; i+=5,k++) {
    ...
    if (fabs(a[i+2])<FPMIN) errormsg("mrqfit_gauss(): Gaussian %d has near zero\n\
\tor negative width (=%lf)",i/5+1,a[i+2]);
    ...
    xma1=x[0]-a[i+1]; xma12=xma1*xma1; sig2=a[i+2]*a[i+2]; fac=xma12/sig2;
    expon=-0.5*fac; ex=(fac<80.0) ? exp(expon) : 0.0;
    *y+=(double)inc[k]*a[i]*ex+(double)inc[k]/(double)ninc*(a[i+3]*xma1+a[i+4]);
    if (dyda!=NULL) {
      dyda[i]=ex;
      dyda[i+1]=a[i]*xma1/sig2*ex-a[i+3];
      dyda[i+2]=a[i]*xma12/sig2/a[i+2]*ex;
      dyda[i+3]=xma1;
      dyda[i+4]=1.0;
    }
  }
