/****************************************************************************
Copyright 2003-2013 Michael Murphy

This file is part of UVES_popler.

UVES_popler is free software; you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation; either version 2 of the License, or
(at your option) any later version.

UVES_popler is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
GNU General Public License for more details.

You should have received a copy of the GNU General Public License
along with UVES_popler; if not, write to the Free Software
...
\tarray of length %d",k);
  ...
  while (!cp->exit) {
	...
	cpgsci((PMOD(con[l][5],3,14,0))); cpgline(con[l][4],&(wl[con[l][2]]),or);
	for (j=0,k=con[l][2]-spec[con[l][0]].or[con[l][1]].csidx; j<con[l][4];
	     j++,k++) or[j]=spec[con[l][0]].or[con[l][1]].rder[k];
	cpgsci((PMOD(con[l][5],1,14,0))); cpgline(con[l][4],&(wl[con[l][2]]),or);
	cpgsci((PMOD(con[l][5],2,14,0)));
      } else cpgsci(3);
...
\tfor fy2 array of size %d",nxyp);
	  ...
	  while (strncmp(pgch,"q",1) && strncmp(pgch,"a",1)) {
	    ...
	    cpgsci((PMOD(con[i][5],1,14,0))); cpgline(cnp,&(wl[sp]),&(co[sp]));
	    /* Plot new continuum */
	    for (j=0,k=fsp-con[i][2]; j<fnp; j++,k++) or[j]=fco[k];
	    cpgsci((PMOD(con[i][5],2,14,0))); cpgline(fnp,&(wl[fsp]),or);
	    /* Mark spline nodes */
	    cpgsch(0.6*plenv.ch);
