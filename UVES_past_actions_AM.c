/****************************************************************************
Copyright 2003-2012 Michael Murphy
...
\texecute (opt=%d) is greater than number available (=%d)",-1*opt,nact); return 0;
  ...
  for (i=sact; i<=eact; i++) {
      ...
      // printf("%05d\n",i+1); //AM!!!
      // printf("index: %d to %d\n", spec[act[i].i[0]].or[act[i].i[1]].csidx, spec[act[i].i[0]].or[act[i].i[1]].ceidx);
      // printf("nuse: %d\n",spec[act[i].i[0]].or[act[i].i[1]].nuse); //AM!!!
...
\tof order %d in file %d,\n\t%s\n\twith order limits: %.10lf-%.10lf\n",i+1,act[i].d[0],act[i].d[1],
act[i].i[1]+1,act[i].i[0]+1,spec[act[i].i[0]].file,   cspec->wl[spec[act[i].i[0]].or[act[i].i[1]].csidx], cspec->wl[spec[act[i].i[0]].or[act[i].i[1]].ceidx]);
act[i].val = 0; // ??? can I do this?
continue; // and this?
   }
