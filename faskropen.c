...
FILE *faskropen(char *query, char *filename, int opt) {
  ...
  if (opt == 1) get_input(query, "!%s", filename);
    ...
    ;
  else errormsg("faskropen(): Unknown option: %d", opt);

  while ((access(filename, R_OK) == -1 || filename[0] == '!') && opt!=4) {
    if (opt == 5) errormsg("faskropen(): Cannot open file %s for reading",
			   filename);
    if (filename[0] == '!') system(filename+1);
    else nferrormsg("faskropen(): Cannot open file file %s for reading",
		    filename);
